...
export const FIELD_DEFS = [
  ...
  { key: 'amount_requested', label: 'Amount Requested', dataType: 'MONETARY', get: (p) => b(p).amountRequested },
  ...
  { key: 'monthly_revenue', label: 'Monthly Revenue', dataType: 'MONETARY', get: (p) => b(p).MonthlyRevenue },
...
for (let i = 1; i <= MAX_COLLATERAL_PROPERTIES; i++) {
  for (const name of collateralFieldNames) {
    FIELD_DEFS.push({
      ...
      dataType: name.includes('Price') || name.includes('Value') || name.includes('Balance') ? 'MONETARY' : 'TEXT',
...
for (let i = 1; i <= MAX_LENDER_POSITIONS; i++) {
  for (const name of lenderFieldNames) {
    FIELD_DEFS.push({
      ...
      dataType: name.includes('Balance') || name.includes('Amount') ? 'MONETARY' : 'TEXT',
